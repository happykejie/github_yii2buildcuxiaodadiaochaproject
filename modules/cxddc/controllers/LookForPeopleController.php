...
class LookForPeopleController extends Controller{
    ...
    public function actionIndex($id = -1,$code=null){
        ...
        {
            $urlexpert = "/cxddc/lookforpeoplebszd/expert?id=".$this->getteacherid;
        }
        else
        {
            $urlexpert = "/cxddc/lookforpeople/expert?id=".$this->getteacherid;
        }
        ...
        if($code){
            ...
            $returl="http://".WWW."/cxddc/lookforpeople/index";//Url::to(['/wx/wxapi/login'],true);
    ...
    public function  actionAuthexpert($id = 1,$code=null)
    {
        ...
        if(CURR_DOMAIN=='boshizhidao.com')
        {
            $urlexpert = "/cxddc/lookforpeoplebszd/expert?id=".$id;
        }
        else
        {
            $urlexpert = "/cxddc/lookforpeople/expert?id=".$id;
        }
        
       
		$urlexpertwww ="http://".WWW."/cxddc/lookforpeople/expert?id=".$id;	
        $this->_user = YiiUser::findOne(['id'=>Yii::$app->user->getId()]);
        if( $this->_user ){
            $this->_openid = $this->_user->openid;
            //返回提问老师页面
            ...
            if(CURR_DOMAIN=='boshizhidao.com')
            {
                $urlexpert = "/cxddc/lookforpeoplebszd/expert?id=".$this->_user->id;
            }
            else
            {
                $urlexpert = "/cxddc/lookforpeople/expert?id=".$this->_user->id;
            }
    ...
    public function actionExpert($id=1,$code=null)
	{
		...
		else
        {
            ...
            return $this->redirect('/cxddc/cxddc/index',['items'=>$items,'items1'=>$items1,'items2'=>$items2,'items3'=>$items3,'items4'=>$items4,'askone'=>$askone,'category'=>$category]);
        }
    }
    /**
     * @找人—显示所有老师
